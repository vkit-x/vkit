...
class Engine(Generic[_T_CONFIG, _T_RESOURCE, _T_RUN_CONFIG, _T_OUTPUT]):
    ...
    def run(self, config: _T_RUN_CONFIG, rnd: RandomState) -> _T_OUTPUT:
        raise NotImplementedError()
...
class EngineRunner(Generic[_T_CONFIG, _T_RESOURCE, _T_RUN_CONFIG, _T_OUTPUT]):
    ...
    def run(self, config: Union[Dict[str, Any], _T_RUN_CONFIG], rnd: RandomState) -> _T_OUTPUT:
        config = dyn_structure(config, self.get_run_config_cls())
        return self.engine.run(config, rnd)
...
class EngineRunnerAggregator(Generic[_T_RESOURCE, _T_RUN_CONFIG, _T_OUTPUT]):
    ...
    def run(self, config: Union[Dict[str, Any], _T_RUN_CONFIG], rnd: RandomState) -> _T_OUTPUT:
        engine_runner = rnd_choice(rnd, self.engine_runners, probs=self.probs)
        return engine_runner.run(config, rnd)
